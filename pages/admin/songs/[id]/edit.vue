    ...
    <div>
      <div class="p-5">
        <form class="w-full flex flex-col items-start justify-start gap-2" @submit.prevent="OnSubmit">
          ...
          <div class="w-full flex flex-col justify-between gap-2 rounded-xl bg-latte-surface0 p-5 shadow xl:flex-row dark:bg-mocha-surface0 dark:shadow-none">
            <div class="w-full flex gap-2">
              ...
              <FileInputField
                ...
                @update:model-value="(v: Array<File>) => EditSongInfo.PreviewFile = v[0]"
              />
            </div>
          </div>
          ...
          <div class="w-full flex flex-col justify-between gap-2 rounded-xl bg-latte-surface0 p-5 shadow xl:flex-row dark:bg-mocha-surface0 dark:shadow-none">
            <div class="w-full flex gap-2">
              <FileInputField
                ...
                @update:model-value="(v: Array<File>) => Info.VoiceFile = v[0]"
              />
              <FileInputField
                class="w-1/2"
                label="Instrumental File"
                name="song-instrumental-file"
                @update:model-value="(v: Array<File>) => Info.InstrumentalFile = v[0]"
              />
            </div>
          </div>
          ...
          <div class="w-full flex flex-col justify-between gap-2 rounded-xl bg-latte-surface0 p-5 shadow xl:flex-row dark:bg-mocha-surface0 dark:shadow-none">
            <div class="w-full flex gap-2">
              <div v-for="(lyrics, idx) in EditSongInfo.Lyrics" :key="lyrics.Id" v-motion-pop class="w-full flex items-center justify-start gap-2">
                <TextAreaInputField
                  v-model="lyrics.Text"
                  :value="lyrics.Text"
                  class="w-1/2"
