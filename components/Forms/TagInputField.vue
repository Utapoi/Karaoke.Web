...
<template>
  <div ref="SelectRef" class="my-1 inline-flex flex-col">
    ...
    <div
      ...
      @click="IsOpen = !IsOpen"
    >
      <div class="w-full flex items-start justify-between gap-4 hover:cursor-pointer">
        <div v-if="Selected.length > 0" class="flex flex-wrap gap-2">
          <div v-for="selected in Selected" :key="selected" class="inline-flex items-center gap-2 whitespace-nowrap rounded-full bg-latte-surface2 px-2 dark:bg-mocha-surface2">
            {{ selected }} <div class="i-fluent:delete-12-filled" @click="OnRemoveSelected(selected)" />
          </div>
        </div>
        <div
