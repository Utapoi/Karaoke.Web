    ...
    <div class="relative max-h-40 rounded-xl shadow dark:bg-[#222222]">
      <div class="w-full flex justify-start">
        <div class="relative h-40 min-h-40 min-w-40 w-40" @mouseenter="IsHover = true" @mouseleave="IsHover = false" @focusout="IsHover = false">
          ...
          <div class="absolute bottom-1 right-1 rounded-full px-2 py-0.5 text-sm font-semibold backdrop-blur-xl dark:bg-black/70 dark:text-white">
            {{ song.duration }}
          </div>
          <NuxtLink :to="`/songs/${song.id}`">
            <img class="h-full w-full rounded-xl object-cover hover:cursor-pointer" :src="song.album.albumArt" alt="Song cover">
          </NuxtLink>
        </div>
        <div class="w-full flex flex-col justify-between overflow-hidden p-3">
          ...
          <div>
            <div class="w-full flex items-center justify-between">
              <NuxtLink :to="`/songs/${song.id}`" class="relative line-clamp-1 block w-full text-ellipsis whitespace-nowrap text-lg font-semibold text-white hover:cursor-pointer">
                {{ song.title }}
              </NuxtLink>
              <div>
            ...
            <NuxtLink :to="`/albums/${song.album.id}`">
